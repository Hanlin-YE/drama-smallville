...
REAL_AUDIENCE_CONFIG_PATH = Path(__file__).resolve().parents[3] / "real_audience" / "profiles" / "audience_distributions.json"
...
def load_distribution(distribution_id: str | None, platform_type: str) -> AudienceDistributionConfig:
    ...
    real_configs = _load_configs(REAL_AUDIENCE_CONFIG_PATH)
    if distribution_id:
